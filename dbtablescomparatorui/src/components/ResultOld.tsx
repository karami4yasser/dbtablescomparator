...
export default function Result(props: ResultColumnsProps) {
  ...
  const [allData, setAllData] = React.useState<string>("All Data");
  ...
  const onFilterChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    setAllData((event.target as HTMLInputElement).value);
    setSelectedTable(selectedTable);
    if (selectedTable) setLoading(true);
    // Fetch data from backend based on selected table and selected columns
    if (selectedTable) fetchData(selectedTable, selectedColumns[selectedTable]);
  };
